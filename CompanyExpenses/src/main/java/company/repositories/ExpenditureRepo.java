...
public interface ExpenditureRepo extends JpaRepository<Expenditure, Integer> {
	...
	Page<Expenditure> findExpensesByCategory(@Param("catCode") String catCode, Pageable pageable);
	...
	@Query(value = "SELECT e FROM Expenditure e WHERE e.paymentCode = :paymentCode")

	Page<Expenditure> findExpensesByPaymentMode(@Param("paymentCode") String paymentCode, Pageable pageable);
